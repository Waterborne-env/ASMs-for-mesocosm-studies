...
# 04-06-2022  A. Lindborg Revises input files to use new "noquantile" files from 11_Setup.. script - this was a request from waterborne to only calculate min and max, not 5th and 95th percentile
# 05-15-2023  A. Lindborg Revise script to create validation grading tables for 2021 studies (application and peak)
# PURPOSE -----------------------------------------------------------------
...
results_folder <- "AQUATOX_006_ASM_RingStudy_Mesocosm_Model_Validation_2021"
...
setwd("Aquatox_Results_Validation_2021")

# LOAD CALIBRATION FILES --------------------------------------------------------------------

crit <- read_csv("Input/calibration_criteria.csv")
crit.study <- read_csv("Input/calibration_criteria_by_study_noquantile.csv")
crit.study.long <- read_csv("Input/calibration_criteria_by_study_long_noquantile.csv")%>%
...
### Dataframe with all model results - use for plotting
...
aq.results.exp <- aq.results.all %>%
  filter(date.obj >= exp.dates$min_date[match(year, exp.dates$year)] & date.obj <= exp.dates$max_date[match(year, exp.dates$year)]) 
#%>%
#add a study column (to differentiate between peak and application scenarios)
# mutate(year_study = "2021_Application")

# #create duplicate results for application and peak scenarios
# aq.results.dup <- aq.results.filt %>%
#   mutate(year_study = "2021_Peak")
# 
# #combine final results - NOTE: the results should be the same, just creating duplicate entries for peak and application scenarios
# aq.results.exp <- bind_rows(aq.results.filt, aq.results.dup)
...
#   #remove duplicate results, otherwise joining won't work in forloop
#   filter(year_study == "2021_Peak") %>%
...
# 
# for(i in c("2021_Application", "2021_Peak")){
#   
#   year.crit <- crit.study.long %>% filter(year_study == i) %>% left_join(., crit)
#   
#   d <- aq.results.exp %>% 
#     filter(year_study == i) %>%
#     left_join(., year.crit) %>%
#     mutate(meets_criteria = ifelse(model_result>= crit_biomass_min & model_result<=crit_biomass_max, TRUE, FALSE),
#            meets_study = ifelse(model_result>= min & model_result<= max, TRUE, FALSE)) %>%
#     group_by(aq_record) %>%
#     summarize(n_tot = n(),
#               n_meet_crit = sum(meets_criteria),
#               per_meet_crit = round((n_meet_crit/n_tot)*100,2),
...
#                        "Total Number of Modeled Points (both studies combined)", 
#                        "No. Modeled Points that Meet Criteria", 
#                        "% Points Meet Calibration Criteria", 
#                        "2021 Peak: % Criteria met", 
#                        "2021 Peak: % Study Met",
#                        "2021 Application: % Criteria met", 
#                        "2021 Application: % Study Met")
# 
# ### Save Workbook:
# wb <- createWorkbook()
...
wb.grade <-  aq.results.exp %>%
  left_join(., crit) %>%
  # #remove study designation because peak and application results should be the same (we didn't model them separately)
  # filter(year_study == "2021_Peak") %>%
  ...
  group_by(aq_record) %>%
  mutate(peak_fulfilled = ifelse(sum(meet_peak)==1, TRUE, FALSE),
         min_fulfilled = ifelse(sum(meet_min)==1, TRUE, FALSE),
         max_fulfilled = ifelse(sum(meet_max)==1, TRUE, FALSE)) %>%
  ungroup() %>%
  pivot_wider(names_from = year, 
              values_from = c("peak_monthday","peak_value" ,"min_value", "max_value","mean_value", "meet_peak", "meet_min", "meet_max")) %>%
  relocate(peak_value_2021, min_value_2021, max_value_2021, mean_value_2021,
           .after = peak_monthday_2021)
...
for(i in unique(wb.grade$parameter_group)) {
  d <- wb.grade %>%
    ...
    dplyr::select(consensus_group:crit_biomass_max, peak_monthday_2021:mean_value_2021) %>%
    ...
    rename(`Consensus Group` = consensus_group,
           ...
           `Peak Date 2021` = peak_monthday_2021,
           `Peak Biomass 2021`= peak_value_2021,
           `Minimum Biomass 2021` = min_value_2021,
           `Maximum Biomass 2021` = max_value_2021,
           `Mean Biomass 2021` = mean_value_2021
    ) %>%
...
saveWorkbook(wb, "Output/006_Waterborne_Validation_Table1.xlsx", overwrite = TRUE)
