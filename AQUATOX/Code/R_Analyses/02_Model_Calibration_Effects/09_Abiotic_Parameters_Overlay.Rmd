---
title: "Physical and chemical parameter overlay file"
author: "A. Lindborg"
date: "5/9/2022"
...
Physical parameters that are AQ state variables:
  - Water volume          + Mesocosm effects data
  - Temperature           + Mesocosm effects data (for enclosure); + Simulated data from gaiac
  - Wind Loading          + MET data  
  - Light                 + MET data
  - pH                    + Mesocosm effects data
  
Chemical Parameters that are AQ state variables:
  - Total Ammonia as N    + Mesocosm effects data
  - Nitrate as N          + Mesocosm effects data  
  - Total Soluble P       + Mesocosm effects data
  - Carbon Dioxide**
  - Oxygen                + Mesocosm effects data
...
# Load and organize parameters from effects Data:
```{r control_params}
# Load effects Mesocosm study data -- filter for variables and clean up units:
cd <- read.csv("Output/01_Compile_Effects_Data/effects_long.csv") %>%
...
# Water Level/ Volume -- convert from L to cubic meters; filter for studies that are in effects data; remove var with liters
