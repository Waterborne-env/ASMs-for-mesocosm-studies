...
# Purpose
Organize data from the effects studies into one master dataset. Data are provided by GmbH.
...
# History
5/5/2022 - A. Lindborg modifies script based on L. Mudge validation control script of same name
...
path19 <- "Data/Effects Data GmbH/Full Study 19b - AZT Study"


# Create a list of file names:
files <- list.files(path =  path19,
                    pattern = "*.csv",
                    full.names = TRUE)
...
control_data_list <- tibble(filename = files) %>% 
  ...
  mutate(units_or_id = 
         ...
         study = ifelse(study == "Full-Study-19b", "Control-19b", study)) 
...
write_csv(out, "Output/01_Compile_Effects_Data/effects_long.csv")
saveRDS(out, file = "Output/01_Compile_Effects_Data/effects_long.rds")
...
``` {r check species}
 ...
 cal <- read.csv("Working_Files/R_Analyses/00_Model_Calibration/Output/01_Compile_Control_Data/control_long.csv") 
 cal <- cal %>% 
   filter(study == "Control-19b") %>% #only compare to 19b control (not other years)
   select(variable)
 ...
 check = setdiff(cal$variable, val$variable)  #no differences between 2019 control and effects data
...
``` {r calib_data}
 ...
 crit <- read.xlsx("Working_Files/R_Analyses/compiled_calibration_crit.xlsx", sheet = "calib_crit", detectDates = TRUE) %>% 
...
# write_csv(dc, "Output/01_Compile_Effects_Data/effects_data_for_calibration.csv") 
# saveRDS(dc, "Output/01_Compile_Effects_Data/effects_data_for_calibration.rds") 

``` 
 
``` {r control_long_with_criteria}
 ...
 #write_csv(out.crit, "Output/01_Compile_Effects_Data/effects_long_wCriteria.csv") 
 #saveRDS(out.crit, "Output/01_Compile_Effects_Data/effects_long_wCriteria.rds")   
