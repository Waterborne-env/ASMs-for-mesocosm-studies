...
# History
5/9/2022 - A. Lindborg modifies script based validation script of same name
...
pp_total <- AQ_PeriPhyton %>%
  ...
  mutate(species_group = ifelse(parameter_group == "Periphyton-Chl-a", "Total Periphyton", "Total Phytoplankton"),
         ...
         max_bar_value = group_biomass) %>%
  ungroup() %>%
  distinct(parameter_group, species_group, study, enclosure, treatment, day, date, group_biomass, .keep_all = TRUE) %>%
